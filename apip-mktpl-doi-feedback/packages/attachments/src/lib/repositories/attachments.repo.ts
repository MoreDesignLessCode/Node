...
export class AttachmentsRepository implements IRepository<Attachments> {
    ...
    constructor(storageProvider: IStorageProvider<Attachments> | undefined) {
        if (!storageProvider) {
            throw new StorageProviderUndefinedError(
                Constants.errors.repository.attachment.storageProvider.undefined.CODE
            )
                .withTitle(
                    Constants.errors.repository.attachment.storageProvider.undefined
                        .TITLE
                )
                .withReason(
                    Constants.errors.repository.attachment.storageProvider.undefined
    ...
    create = async (resource: Attachments, context: IContext): Promise<Result<Attachments>> =>
